...
class ProjectForm(forms.Form):
  ...
  created_at = forms.DateTimeField(label="Date created at", required=False)
  updated_at = forms.DateTimeField(label="Date updated at", required=False)
  languages = forms.CharField(label="Programming Languages", max_length=1000, required=False)
  speciality = forms.CharField(label="Speciality", max_length=100, required=False)
  years = forms.IntegerField(label="Years of Programming experience", min_value=0, required=False)

class updateStatus(forms.Form):
  status = forms.IntegerField(label="status", min_value=0, max_value=100, required=False)
